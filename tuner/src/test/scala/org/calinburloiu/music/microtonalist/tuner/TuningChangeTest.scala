...
class TuningChangeTest extends AnyWordSpec with Matchers {

  "isTriggering" should {
    "return the correct value depending on the case instance" in {
  ...
  "IndexTuningChange" should {
    "return the correct value for index" in {
      IndexTuningChange(0).index shouldEqual 0
      IndexTuningChange(1).index shouldEqual 1
    }

    "throw IllegalArgumentException if index is negative" in {
