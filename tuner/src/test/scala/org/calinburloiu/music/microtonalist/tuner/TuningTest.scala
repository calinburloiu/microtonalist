...
class TuningTest extends AnyWordSpec with Matchers {
  ...
  "constructor" should {
    "throw IllegalArgumentException when the number of offsets is not 12" in {
  ...
  "apply, size and iterator" should {
    "work as an indexed sequence" in {
  ...
  "note names implicit methods" should {
    "return the correct offsets" in {
      val tuning = Tuning(
  ...
  "isComplete" should {
    "return true if all offset are non-empty" in {
  ...
  "enrich" should {
    "correctly do a best effort combine of Tunings" in {
