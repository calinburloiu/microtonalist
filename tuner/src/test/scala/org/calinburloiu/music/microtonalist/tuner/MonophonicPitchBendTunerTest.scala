...
class MonophonicPitchBendTunerTest extends AnyWordSpec with Matchers with Inside {
  ...
  "MonophonicPitchBendTuner on initialization" should {
    "fail if constructed with out of bounds output channel" in {
    ...
    "configure the output device" in {
  ...
  "MonophonicPitchBendTuner after initialization" should {
    "tune all notes in 12-EDO by not sending any pitch bend" in new Fixture {
    ...
    "not send pitch bend when no note is on even if the tuning of C is changed" in new Fixture {
  ...
  "MonophonicPitchBendTuner after a new tuning is set" should {
    "not send pitch bend when the last note is replayed and its tuning did not change" in new Fixture {
    ...
    "send pitch bend when playing a note with a different tuning than the previous one" in new Fixture {
      output ++= tuner.tune(customTuning)
    ...
    "not send pitch bend for consecutive notes with the same tuning" in new Fixture {
  ...
  "MonophonicPitchBendTuner when the tuning is changed" should {
    "not send pitch bend if a note is on and its tuning does not change" in new Fixture {
  ...
  "MonophonicPitchBendTuner when multiple notes are on" should {
    ...
    "always revert to the last note played while releasing simultaneous notes one by one" in new Fixture {
      tuner.tune(customTuning)
    ...
    "tune reverted notes when holding a non-microtonal note while playing a microtonal one and lifting it" in
      new Fixture {
        tuner.tune(customTuning)
    ...
    "keep a re-pressed note sounding until its last Note Off, then revert to the note still held" in
      new Fixture {
        // Given
    ...
    "keep a re-pressed note sounding until its last velocity-0 Note On, then revert to the note still held" in
      new Fixture {
        // Given
        // C4 is pressed, E4 takes over, then C4 is pressed a second time without the first press being released.
        tuner.tune(customTuning)
    ...
    "emit nothing when a note that is not the sounding one is released press by press" in new Fixture {
      // Given
  ...
  "MonophonicPitchBendTuner when it receives pitch bend messages" should {
    "only add the pitch bend received if the note played is not microtonal" in new Fixture {
      sendNote(noteC4)
    ...
    "add the pitch bend received to the one computed for tuning a microtonal note" in new Fixture {
      tuner.tune(customTuning)
    ...
    "add the pitch bend received to the one computed for tuning the last microtonal note (which is off)" in
      new Fixture {
        tuner.tune(customTuning)
        sendNote(noteE4)
    ...
    "continue adding the last pitch bend received to the one for notes of different tunings" in new Fixture {
  ...
  "MonophonicPitchBendTuner when non-tuning-related MIDI messages are received" should {
    "forward modulation CC message on the correct channel" in new Fixture {
    ...
    "forward a System Real-Time message unchanged" in new Fixture {
      // Given
      tuner.tune(customTuning2)
  ...
  "MonophonicPitchBendTuner when pedals are depressed" should {
    "interrupt sustain pedal in order to not violate monophony" in new Fixture {
      tuner.tune(customTuning)
    ...
    "interrupt sustain pedal when holding notes in order to not violate monophony" in new Fixture {
      tuner.tune(customTuning)
    ...
    "stop sostenuto pedal in order to not violate monophony" in new Fixture {
      tuner.tune(customTuning)
    ...
    "immediately send updated pitch bend if sensitivity changes while a note is on" in new Fixture {
      tuner.tune(customTuning)
  ...
  "MonophonicPitchBendTuner for playing microtonal notes" should {
    for (pbs <- pitchBendSensitivities) {
      val pbsString: String = stringOfPitchBendSensitivity(pbs)

      s"play a scale with microtonal notes when pitch bend sensitivity is $pbsString" in new Fixture(pbs) {
        tuner.tune(customTuning)
  ...
  "MonophonicPitchBendTuner bugs" should {
    "work for note Bb on, note A on, note Bb off, note Bb on in custom tuning" in new Fixture {
      tuner.tune(customTuning)
    ...
    "revert to a still-held note after re-pressing then releasing an already-played note twice" in
      new Fixture {
