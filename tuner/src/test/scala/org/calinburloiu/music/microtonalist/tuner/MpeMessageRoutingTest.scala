 ...
 * == Test Organization ==
 *
 * One `should` block per public function. The `route` block is the paper's message-handling table ("How the MPE
 ...
 */
class MpeMessageRoutingTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  ...
  "MpeMessageRouting.roleOf" should {
    // ---- MPE Input Mode ----

    "classify every channel of a dual-Zone configuration" in {
      // Given
    ...
    "classify a channel of no enabled Zone as Outside" in {
      // Given
    ...
    "classify every channel as Outside when no Zone is enabled" in {
      // Given
    ...
    "classify every channel as NonMpeInput of the Lower Zone when it is enabled" in {
      // Given
    ...
    "fall back to the Upper Zone only when it is enabled" in {
      // When / Then
      MpeMessageRouting.roleOf(MpeInputMode.NonMpe, upperOnlyZones, 3) shouldEqual MpeChannelRole.NonMpeInput(upper7)
    }

    "classify every channel as Outside when no Zone is enabled in Non-MPE Input Mode" in {
      // When / Then
  ...
  "MpeMessageRouting.route" should {
    // ---- Channel messages ----

    "route the message classes of the paper's table" in {
      // Given
      val verdicts = Table(
    ...
    "forward a Master Channel message on the Zone's Master Channel" in {
      // Given
    ...
    "forward an Upper Zone Master Channel message on the Upper Zone Master Channel" in {
      // Given
    ...
    "redirect a Non-MPE input message to the Upper Zone Master Channel only when it is enabled" in {
      // Given
    ...
    "interpret an MCM Data Entry MSB on MIDI Channel 1 or 16 (1-based) whatever the role" in {
      // Given
    ...
    "discard an MCM Data Entry MSB requesting more Member Channels than a Zone can hold" in {
      // Given
      val roles = Table("role", memberRole, masterRole, nonMpeRole, outsideRole)
      val channels = Table("channel", 0, 15)
    ...
    "interpret a PBS Data Entry at every role but Outside" in {
      // Given
    ...
    "consume the selector CCs of an interpreted parameter as well" in {
      // Given
    ...
    "consume every RPN and NRPN selector CC" in {
      // Given
    ...
    "re-emit a complete sequence for a data value of an uninterpreted parameter" in {
      // Given
    ...
    "discard a data value when no parameter is selected" in {
    ...
    "re-emit a sequence for a data value of an NRPN whose MSB or LSB is 127" in {
    ...
    "route an NRPN that shares the numbers of an interpreted RPN as uninterpreted" in {
      // Given
      val selectors = Table("selector",
    ...
    "discard an MCM Data Entry received on a channel other than MIDI Channel 1 or 16 (1-based)" in {
      // Given
    ...
    "discard a Data Entry LSB and a Data Increment or Decrement of the MCM" in {
      // Given
    ...
    "discard a Data Increment or Decrement of Pitch Bend Sensitivity at every role" in {
      // Given
  ...
  "MpeMessageRouting.deselectsOnRelay" should {
    "hold for Reset All Controllers alone" in {
      // When / Then
  ...
  "MpeMessageRouting.rpnSequence" should {
    "render an RPN selector ahead of its value message" in {
      // When
    ...
    "render an NRPN selector ahead of its value message" in {
      // When
    ...
    "render nothing when no parameter is selected" in {
      // When
    ...
    "omit the selector when the output channel already holds the parameter selected" in {
      // Given
      val selectors = Table("selector", fineTuningSelector, nrpnSelector)
      forAll(selectors) { selector =>
        // When
    ...
    "render the selector when the output channel holds a different parameter selected" in {
      // When
      val (messages, latchedSelector) = MpeMessageRouting.rpnSequence(fineTuningSelector,
        receivedValueCc(MidiCc.DataEntryMsb, 64), outputChannel = 0, latchedSelector = nrpnSelector)
      // Then
    ...
    "render the selector when the output channel was left deselected by an RPN Null" in {
