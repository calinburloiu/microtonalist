...
class MpeZoneTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  ...
  "MpeZone" should {
    "create a Lower Zone with 15 member channels" in {
      // When
    ...
    "reject a member channel count no Zone can hold" in {
      // When / Then
    ...
    "create a Lower Zone with 7 member channels" in {
      // When
    ...
    "create an Upper Zone with 7 member channels" in {
      // When
    ...
    "create a Lower Zone with 3 member channels" in {
      // When
    ...
    "create a Lower Zone with 2 member channels" in {
      // When
    ...
    "create a Lower Zone with 1 member channel" in {
      // When
    ...
    "create a disabled Lower Zone with 0 member channels" in {
      // When
    ...
    "create a disabled Upper Zone with 0 member channels" in {
      // When
    ...
    "not overlap channels for Lower 7 + Upper 7" in {
      // Given
    ...
    "match Appendix A table for all n from 1 to 15" in {
      //@formatter:off
      val table = Table(
  ...
  "MpeZones" should {
    "preserve both zones when there is no overlap" in {
      // When
    ...
    "shrink lower zone on construction when zones overlap" in {
      // When
    ...
    "disable lower zone on construction when upper zone takes all channels" in {
      // When
    ...
    "preserve pitch bend sensitivities of lower zone when shrunk on construction" in {
      // Given
    ...
    "fail to construct with wrong zone types" in {
      // When / Then
  ...
  "MpeZones.update" should {
    "replace lower zone without affecting upper when no overlap" in {
      // Given
      val zones = MpeZones(MpeZone(MpeZoneType.Lower, 7), MpeZone(MpeZoneType.Upper, 7))
      // When
    ...
    "replace upper zone without affecting lower when no overlap" in {
      // Given
      val zones = MpeZones(MpeZone(MpeZoneType.Lower, 7), MpeZone(MpeZoneType.Upper, 7))
      // When
    ...
    "shrink upper zone when updating lower causes overlap" in {
      // Given
      val zones = MpeZones(MpeZone(MpeZoneType.Lower, 7), MpeZone(MpeZoneType.Upper, 7))
      // When
    ...
    "shrink lower zone when updating upper causes overlap" in {
      // Given
      val zones = MpeZones(MpeZone(MpeZoneType.Lower, 7), MpeZone(MpeZoneType.Upper, 7))
      // When
    ...
    "preserve pitch bend sensitivities of other zone when shrunk" in {
      // Given
    ...
    "disable other zone when updated zone takes all channels" in {
      // Given
      val zones = MpeZones(MpeZone(MpeZoneType.Lower, 7), MpeZone(MpeZoneType.Upper, 7))
      // When
    ...
    "follow the full example flow from the requirements" in {
      // Given
