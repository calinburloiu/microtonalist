...
class TuningSessionTest extends AnyWordSpec with Matchers with MockFactory {
  ...
  "constructor" should {
    "initialize with empty tunings and tuningIndex set to 0" in new Fixture {
  ...
  "tuningIndex" should {
    "throw an exception when setting an invalid tuningIndex" in new Fixture {
      // When
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast)
      // Then
    ...
    "allow setting a valid tuningIndex" in new Fixture {
      // Given
    ...
    "allow setting a value >= the sequence size and set it to the last index" in new Fixture {
      // Given
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast, TestTunings.justDUssak)
      // When
    ...
    "publish TuningIndexUpdatedEvent when tuningIndex changes" in new Fixture {
      // Given
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast)
      // When
    ...
    "not publish TuningIndexUpdatedEvent when tuningIndex remains the same" in new Fixture {
      // Given
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast)
      // When
  ...
  "tunings" should {
    "set tunings and adjust tuningIndex if necessary" in new Fixture {
      // When
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast)
      // Then
      tuningSession.tunings shouldBe Seq(TestTunings.justCMaj, TestTunings.justCRast)
    ...
    "publish TuningsUpdatedEvent when tunings are set" in new Fixture {
      // When
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast)
    ...
    "not publish TuningsUpdatedEvent when the same tunings is set" in new Fixture {
      // Given
  ...
  "previousTuning, nextTuning and nextBy" should {
    "cycle to the previous and next tunings correctly" in new Fixture {
      tuningSession.tunings = Seq(TestTunings.justCMaj, TestTunings.justCRast, TestTunings.justDUssak)
