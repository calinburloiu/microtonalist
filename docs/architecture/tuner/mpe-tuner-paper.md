...
4. **Redirection of Remaining Channel Messages**: All other Channel Voice and Channel Mode messages received on a
   ...
   complete sequence, save for a value message received while no parameter is completely selected, which is
   discarded (Section 4).
...
| All other RPN and all NRPN messages | Discarded | Re-emitted as a complete sequence on the same Master Channel, unless no parameter is completely selected (Section 4) |
