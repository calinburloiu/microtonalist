...
sequence). `RpnMessages` is the home of the Registered and Non-Registered Parameter message vocabulary: it names the
parameters Microtonalist selects (Pitch Bend Sensitivity, the MPE Configuration Message, and the Null Function as an
RPN and as an NRPN) and renders a selector as its pair of Control Change messages, so that the transmission order of
