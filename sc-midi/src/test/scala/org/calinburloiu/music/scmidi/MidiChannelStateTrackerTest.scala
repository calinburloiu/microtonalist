...
class MidiChannelStateTrackerTest extends AnyWordSpec with Matchers {
  ...
  "MidiChannelStateTracker per note tracking" should {
    "have no active notes on any channel when empty" in new TrackerFixture {
      // When / Then
      for (channel <- 0 to 15) {
    ...
    "record a Note On as an active note with its velocity" in new TrackerFixture {
      // When
    ...
    "remove a note from the active set on Note Off" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "treat a Note On with velocity 0 as a Note Off" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "order active notes by their Note On" in new TrackerFixture {
      // When
    ...
    "track active notes independently per channel" in new TrackerFixture {
      // When
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "default Polyphonic Key Pressure to 0 for an active note that has not received one" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "update Polyphonic Key Pressure for an active note" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "ignore Polyphonic Key Pressure for an inactive note" in new TrackerFixture {
      // When
      tracker.send(PolyPressureMidiMsg(Channel, C4, value = 90))
    ...
    "preserve Polyphonic Key Pressure when a note is re-triggered with Note On" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(PolyPressureMidiMsg(Channel, C4, value = 90))
    ...
    "overwrite the velocity of an active note when a Note On is re-sent" in new TrackerFixture {
      // Given
    ...
    "count a single Note On as one reference" in new TrackerFixture {
      // When
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "increment the reference count when an already-active note receives another Note On" in
      new TrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "decrement the reference count on Note Off while a reference remains, keeping the note active" in
      new TrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "remove the note when the last Note On is discharged" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
      tracker.send(NoteOffMidiMsg(Channel, C4))
    ...
    "decrement the reference count on a Note On with velocity 0 exactly as on a Note Off" in
      new TrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "leave the reference count at 0 when a Note Off arrives for an inactive note" in new TrackerFixture {
      // When
      tracker.send(NoteOffMidiMsg(Channel, C4))
    ...
    "report a reference count of 0 for a note that was never played" in new TrackerFixture {
      // When / Then
      tracker.referenceCount(Channel, C4) should equal(0)
    }

    "track reference counts independently per channel and per note" in new TrackerFixture {
      // When
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "move a note to the end of the ordered active notes on a duplicate Note On" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
  ...
  "MidiChannelStateTracker Control Change tracking" should {
    "return None for ccOption when the CC has not been set" in new TrackerFixture {
      // When / Then
      tracker.ccOption(Channel, MidiCc.ModulationMsb) shouldBe None
    }

    "record the value of a Control Change message" in new TrackerFixture {
      // When
    ...
    "track CC values independently per channel" in new TrackerFixture {
      // When
    ...
    "fall back to the companion's DefaultCcValues for known CCs when nothing is recorded" in
      new TrackerFixture {
        // When / Then
    ...
    "throw NoSuchElementException for an unknown CC with no value, override, or default" in new TrackerFixture {
      // Given
      val unknownCc = 50
    ...
    "reject ccDefaults keyed by a number that is not a controller number" in {
    ...
    "use overrideDefaultValue when set and no value was recorded" in new TrackerFixture {
      // Given
      val unknownCc = 50
    ...
    "prefer the recorded value over override and defaults" in new TrackerFixture {
      // Given
    ...
    "honor a constructor-supplied ccDefault for an unknown CC" in {
      // Given
      val unknownCc = 50
    ...
    "let constructor-supplied ccDefaults override the companion's defaults" in {
      // Given
  ...
  "MidiChannelStateTracker Channel Pressure / Pitch Bend / Program Change tracking" should {
    "default Channel Pressure / Pitch Bend / Program Change to 0" in new TrackerFixture {
      // When / Then
    ...
    "record the latest Channel Pressure value" in new TrackerFixture {
      // When
    ...
    "record the latest Pitch Bend value (signed)" in new TrackerFixture {
      // When
    ...
    "record the latest Program Change value" in new TrackerFixture {
      // When
    ...
    "track Channel Pressure / Pitch Bend / Program Change independently per channel" in new TrackerFixture {
      // When
      tracker.send(ChannelPressureMidiMsg(Channel, value = 80))
  ...
  "MidiChannelStateTracker bankSelect" should {
    "default Bank Select to (0, 0) when nothing is recorded" in new TrackerFixture {
      // When / Then
      tracker.bankSelect(Channel) should equal((0, 0))
    }

    "reflect Bank Select MSB and LSB recorded via CC messages" in new TrackerFixture {
      // When
    ...
    "honour constructor-supplied Bank Select defaults" in {
      // Given
  ...
  "MidiChannelStateTracker RPN tracking" should {
    "return None for an RPN that has not been written" in new TrackerFixture {
      // When / Then
      tracker.rpnOption(Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb) shouldBe None
    }

    "throw NoSuchElementException for rpn() when no value, override, or default is available" in
      new TrackerFixture {
        // When / Then
        a[NoSuchElementException] should be thrownBy tracker.rpn(Channel, 5, 9)
      }

    "return the companion default for rpn() when no value has been recorded" in new TrackerFixture {
      // When / Then
    ...
    "use overrideDefaultValue for rpn() when no value has been recorded" in new TrackerFixture {
      // When / Then
    ...
    "initially have no RPN/NRPN selected" in new TrackerFixture {
      // Then
    ...
    "select a RPN" in new TrackerFixture {
      // When
    ...
    "select a RPN when LSB is sent before MSB (reversed order)" in new TrackerFixture {
    ...
    "deselect the parameter when the Null RPN is selected MSB before LSB" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "deselect the parameter when the Null RPN is selected LSB before MSB" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "keep a lone RPN MSB of 127 pending rather than reading it as a Null" in new TrackerFixture {
    ...
    "initially have no RPN/NRPN partially assembled" in new TrackerFixture {
      // When / Then
      tracker.partialRpnSelector(Channel) shouldEqual PartialRpnSelector.None
    }

    "report an RPN whose LSB has not arrived as a half still pending" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.RpnMsb, MidiRpn.FineTuningMsb))
    ...
    "report an RPN whose MSB has not arrived as a half still pending" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.RpnLsb, MidiRpn.FineTuningLsb))
    ...
    "tell a lone RPN half carrying 127 apart from a Null that deselects" in new TrackerFixture {
    ...
    "start a fresh partial RPN rather than inherit a pending NRPN half" in new TrackerFixture {
    ...
    "track the partially assembled RPN independently per channel" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.RpnMsb, MidiRpn.FineTuningMsb))
    ...
    "record an RPN value via Data Entry MSB after RPN selection" in new TrackerFixture {
      // Given
    ...
    "update the LSB component of an RPN value via Data Entry LSB" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "seed the unrecorded RPN value from the resolved default when only Data Entry LSB is sent" in
      new TrackerFixture {
        // Given
        selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "record an RPN value for a parameter whose LSB is 127" in new TrackerFixture {
    ...
    "keep recorded RPN values when a different RPN is selected" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
    ...
    "ignore data changes when an initial RPN has only MSB selection" in new TrackerFixture {
      // Given
    ...
    "ignore data changes when an initial RPN has only LSB selection" in new TrackerFixture {
      // Given
    ...
    "ignore data changes when no RPN/NRPN is selected" in new TrackerFixture {
      // When
    ...
    "ignore Data Entry after Null RPN is selected" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
    ...
    "track RPN values independently per channel" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 12))
    ...
    "increment the recorded RPN value by 1 (combined 14-bit)" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 64))
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryLsb, value = 50))
    ...
    "decrement the recorded RPN value by 1 (combined 14-bit)" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 64))
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryLsb, value = 50))
    ...
    "carry across LSB and MSB when incrementing past 127 LSB" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "carry across MSB and LSB when decrementing past 0 LSB" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "clamp at 0 when decrementing below 0" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 0))
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryLsb, value = 0))
    ...
    "clamp at 16383 when incrementing past the 14-bit maximum" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "ignore the data byte of Data Increment / Decrement (always ±1)" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 64))
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryLsb, value = 50))
    ...
    "seed the value from the companion's DefaultRpnValues when nothing is recorded for Data Increment" in
      new TrackerFixture {
        // Given
        selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
    ...
    "seed the value from the companion's DefaultRpnValues when nothing is recorded for Data Decrement" in
      new TrackerFixture {
        // Given
        selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
    ...
    "prefer the constructor's rpnDefaults over the companion's" in {
      // Given
      val tracker = MidiChannelStateTracker(
        rpnDefaults = Map(
    ...
    "ignore Data Increment when no RPN/NRPN is selected" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.DataIncrement, value = 0))
    ...
    "ignore Data Increment for an unknown RPN with no recorded value and no default" in new TrackerFixture {
      // Given
      val unknownRpnMsb = 5
      val unknownRpnLsb = 9
    ...
    "ignore Data Decrement for an unknown RPN with no recorded value and no default" in new TrackerFixture {
      // Given
      val unknownRpnMsb = 5
      val unknownRpnLsb = 9
      selectRpn(tracker, Channel, unknownRpnMsb, unknownRpnLsb)
    ...
    "still record Data Increment / Decrement values even when their effect is ignored" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.DataIncrement, value = 0))
    ...
    "let RPN selection take over a previous NRPN selection" in new TrackerFixture {
      // Given
  ...
  "MidiChannelStateTracker NRPN tracking" should {
    "return None for an NRPN that has not been written" in new TrackerFixture {
      // When / Then
      tracker.nrpnOption(Channel, NrpnA._1, NrpnA._2) shouldBe None
    }

    "throw NoSuchElementException for nrpn() when no value, override, or default is available" in
      new TrackerFixture {
        // When / Then
        a[NoSuchElementException] should be thrownBy tracker.nrpn(Channel, NrpnA._1, NrpnA._2)
      }

    "use overrideDefaultValue for nrpn() when no value has been recorded" in new TrackerFixture {
      // When / Then
      tracker.nrpn(Channel, NrpnA._1, NrpnA._2, overrideDefaultValue = Some((7, 0))) should equal((7, 0))
    }

    "select a NRPN" in new TrackerFixture {
      // When
    ...
    "select a NRPN when LSB is sent before MSB (reversed order)" in new TrackerFixture {
      // When — LSB first, then MSB
    ...
    "deselect the parameter when the Null NRPN is selected MSB before LSB" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
    ...
    "deselect the parameter when the Null NRPN is selected LSB before MSB" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
    ...
    "keep a lone NRPN MSB of 127 pending rather than reading it as a Null" in new TrackerFixture {
      // When — only the pair 127/127 is the Null Function; a lone Null MSB deselects nothing on its own
      tracker.send(CcMidiMsg(Channel, MidiCc.NrpnMsb, MidiNrpn.NullMsb))
    ...
    "report an NRPN whose LSB has not arrived as a half still pending" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.NrpnMsb, NrpnA._1))
    ...
    "report an NRPN whose MSB has not arrived as a half still pending" in new TrackerFixture {
      // When
      tracker.send(CcMidiMsg(Channel, MidiCc.NrpnLsb, NrpnA._2))
    ...
    "start a fresh partial NRPN rather than inherit a pending RPN half" in new TrackerFixture {
    ...
    "record an NRPN value via Data Entry MSB after NRPN selection" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
    ...
    "record an NRPN value for a parameter whose MSB is 127" in new TrackerFixture {
    ...
    "record an NRPN value for a parameter whose LSB is 127" in new TrackerFixture {
    ...
    "let NRPN selection take over a previous RPN selection" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
    ...
    "keep recorded NRPN values when a different NRPN is selected" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 1))
    ...
    "ignore Data Entry after Null NRPN is selected" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 8))
    ...
    "ignore data changes when an initial NRPN has only MSB selection" in new TrackerFixture {
      // Given
    ...
    "ignore data changes when an initial NRPN has only LSB selection" in new TrackerFixture {
      // Given
    ...
    "increment a recorded NRPN value for a parameter whose LSB is 127" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, msb = 0, lsb = MidiNrpn.NullLsb)
      tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 17))
    ...
    "increment a recorded NRPN value with constructor-supplied default" in {
      // Given
      val tracker = MidiChannelStateTracker(
    ...
    "decrement a recorded NRPN value with constructor-supplied default" in {
      // Given
      val tracker = MidiChannelStateTracker(
        nrpnDefaults = Map(NrpnA -> (10, 5))
      )
    ...
    "apply multiple Increments and a Decrement to an NRPN value" in {
      // Given
      val tracker = MidiChannelStateTracker(
        nrpnDefaults = Map(NrpnA -> (10, 5))
      )
    ...
    "ignore Data Increment for NRPN with neither recorded value nor default" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
    ...
    "ignore Data Decrement for NRPN with neither recorded value nor default" in new TrackerFixture {
      // Given
      selectNrpn(tracker, Channel, NrpnA._1, NrpnA._2)
  ...
  "MidiChannelStateTracker channel validation" should {
    "throw on activeNotes with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on orderedActiveNotes with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on isNoteActive with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on referenceCount with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on velocity / velocityOption / polyPressure / polyPressureOption with an invalid channel" in
      new TrackerFixture {
        // When / Then
    ...
    "throw on ccOption / cc with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on channelPressure / pitchBend / programChange with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on bankSelect with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on rpnOption / nrpnOption with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on rpnSelector / partialRpnSelector with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on the mode and Local Control accessors with an invalid channel" in new TrackerFixture {
      // When / Then
    ...
    "throw on reset(channel) with an invalid channel" in new TrackerFixture {
      // When / Then
  ...
  "MidiChannelStateTracker Channel Mode messages" should {
    "cancel active notes on the channel when All Sound Off is received" in new ResettableTrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "cancel active notes on the channel when All Notes Off is received" in new ResettableTrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(OtherChannel, G4, velocity = 90))
    ...
    "clear the reference counts of the channel's notes when All Sound Off is received" in
    ...
    "clear the reference counts of the channel's notes when All Notes Off is received" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "clear resettable CCs when Reset All Controllers is received" in new ResettableTrackerFixture {
      // Given
    ...
    "clear Data Entry, Data Increment, and Data Decrement CCs when Reset All Controllers is received" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 12))
    ...
    "clear Channel Pressure, Pitch Bend, and the RPN/NRPN selector when Reset All Controllers is received" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(ChannelPressureMidiMsg(Channel, value = 80))
    ...
    "leave reference counts intact while zeroing Polyphonic Key Pressure on Reset All Controllers" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "clear a half-assembled RPN when Reset All Controllers is received" in new ResettableTrackerFixture {
    ...
    "clear the RPN/NRPN selector so subsequent Data Entry is ignored after Reset All Controllers" in
      new ResettableTrackerFixture {
        // Given
        selectRpn(tracker, Channel, MidiRpn.PitchBendSensitivityMsb, MidiRpn.PitchBendSensitivityLsb)
        tracker.send(CcMidiMsg(Channel, MidiCc.DataEntryMsb, value = 12))
    ...
    "preserve Bank Select, Volume, Pan, Program Change, and RPN/NRPN values on Reset All Controllers" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(CcMidiMsg(Channel, MidiCc.BankSelectMsb, value = 3))
    ...
    "reset Polyphonic Key Pressure on every active note when Reset All Controllers is received" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, E4, velocity = 110))
    ...
    "scope Reset All Controllers to the channel it was received on" in new ResettableTrackerFixture {
      // Given
      tracker.send(CcMidiMsg(Channel, MidiCc.ModulationMsb, value = 64))
    ...
    "not cancel active notes on All Sound Off by default" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "not cancel active notes on All Notes Off by default" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 105))
      tracker.send(NoteOnMidiMsg(Channel, E4, velocity = 110))
    ...
    "not cancel active notes on the MIDI Mode messages 124-127 by default" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 105))
    ...
    "not clear controller state on Reset All Controllers by default" in new TrackerFixture {
      // Given
      tracker.send(CcMidiMsg(Channel, MidiCc.ModulationMsb, value = 64))
      tracker.send(ChannelPressureMidiMsg(Channel, value = 80))
      tracker.send(PitchBendMidiMsg(Channel, value = 1234))
    ...
    "reject a Channel Mode number as a Control Change number in ccOption / cc" in new TrackerFixture {
    ...
    "cancel active notes on the channel when a MIDI Mode message 124-127 is received" in
      new ResettableTrackerFixture {
        // Given
    ...
    "leave active notes and controller values untouched on Local Control" in new ResettableTrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(CcMidiMsg(Channel, MidiCc.ModulationMsb, value = 64))
    ...
    "start every channel in Omni On/Poly mode (Mode 1) with Local Control on" in new TrackerFixture {
    ...
    "track Omni Mode Off and Omni Mode On per channel" in new TrackerFixture {
      // When
      tracker.send(OmniModeOffMidiMsg(Channel))
    ...
    "track Mono Mode On and Poly Mode On per channel" in new TrackerFixture {
      // When
      tracker.send(MonoModeOnMidiMsg(Channel, channelCount = 4))
    ...
    "track the channel count a Mono Mode On asks for, until a Poly Mode On" in new TrackerFixture {
    ...
    "keep the Omni and the Poly/Mono flags independent of each other" in new TrackerFixture {
    ...
    "track Local Control per channel" in new TrackerFixture {
      // When
      tracker.send(LocalControlMidiMsg(Channel, isOn = false))
    ...
    "leave the mode and Local Control untouched by All Sound Off, Reset All Controllers and All Notes Off" in
      new ResettableTrackerFixture {
        // Given
        tracker.send(OmniModeOffMidiMsg(Channel))
        tracker.send(MonoModeOnMidiMsg(Channel, channelCount = 4))
  ...
  "MidiChannelStateTracker reset" should {
    "clear all per-channel state across all channels" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(OtherChannel, E4, velocity = 110))
      tracker.send(CcMidiMsg(Channel, MidiCc.VolumeMsb, value = 90))
    ...
    "clear reference counts across all channels" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "restore the power-up mode and Local Control across all channels" in new TrackerFixture {
      // Given
      tracker.send(OmniModeOffMidiMsg(Channel))
    ...
    "clear the RPN/NRPN selector so subsequent Data Entry is ignored after reset" in new TrackerFixture {
      // Given
      selectRpn(tracker, Channel, MidiRpn.FineTuningMsb, MidiRpn.FineTuningLsb)
    ...
    "clear the state of a single channel, leaving the other fifteen untouched" in new TrackerFixture {
      // Given
      tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
    ...
    "clear the reference counts of a single channel, leaving the other fifteen untouched" in
      new TrackerFixture {
        // Given
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 100))
        tracker.send(NoteOnMidiMsg(Channel, C4, velocity = 110))
    ...
    "restore the power-up mode and Local Control of a single channel, leaving the other fifteen untouched" in
      new TrackerFixture {
        // Given
