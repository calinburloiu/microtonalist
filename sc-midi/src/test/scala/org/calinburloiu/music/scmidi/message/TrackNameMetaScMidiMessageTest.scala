...
class TrackNameMetaScMidiMessageTest extends AnyFlatSpec with Matchers {
  private val name = "Piano"
  private val bytes = name.getBytes("ISO-8859-1")
  private val javaMessage: MidiMessage = {
    val m = new MetaMessage()
    m.setMessage(0x03, bytes, bytes.length)
    m
  }
  ...
  it should "create correct Java MIDI message" in {
    // When / Then
    TrackNameMetaScMidiMessage(name).toJavaMidiMessage.getMessage should equal(javaMessage.getMessage)
  }
