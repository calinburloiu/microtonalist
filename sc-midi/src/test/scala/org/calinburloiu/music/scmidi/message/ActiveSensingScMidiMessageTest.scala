...
class ActiveSensingScMidiMessageTest extends AnyFlatSpec with Matchers {
  ...
  it should "create correct Java MIDI message" in {
    // When / Then
    ActiveSensingScMidiMessage.asJava.getStatus should equal(ShortMessage.ACTIVE_SENSING)
    ActiveSensingScMidiMessage.asJava.getMessage should equal(javaMessage.getMessage)
  }
