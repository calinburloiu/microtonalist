...
class ContinueScMidiMessageTest extends AnyFlatSpec with Matchers {
  private val javaMessage: MidiMessage = new ShortMessage(ShortMessage.CONTINUE)

  behavior of "ContinueScMidiMessage"

  it should "create correct Java MIDI message" in {
    // When / Then
    ContinueScMidiMessage.toJavaMidiMessage.getStatus should equal(ShortMessage.CONTINUE)
    ContinueScMidiMessage.toJavaMidiMessage.getMessage should equal(javaMessage.getMessage)
  }
