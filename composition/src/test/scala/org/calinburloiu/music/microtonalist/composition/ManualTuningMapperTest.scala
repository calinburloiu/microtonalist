...
class ManualTuningMapperTest extends AnyWordSpec with Matchers {
  ...
  "mapScale" should {
    ...
    "map a scale with just quarter tones to custom keys" in {
      // Given
    ...
    "map a large scale that can't be automatically mapped" in {
      // Given
    ...
    "fail to map a scale if an offset overflows" in {
      // Given
    ...
    "fail to map scale if the mapping scale pitches indexes are out of bounds" in {
      // Given
