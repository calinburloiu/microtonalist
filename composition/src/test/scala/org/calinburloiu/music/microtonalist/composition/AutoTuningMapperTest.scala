...
class AutoTuningMapperTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  ...
  "mapScale" should {
    "map a just major scale with a custom tuning reference to a Tuning" in {
    ...
    "transpose a scale before mapping it" in {
      // Given
    ...
    "prepend tuning name with the tuning base pitch class when the scale has a unison" in {
      // Given
    ...
    "not prepend tuning name with the tuning base pitch class when the scale does not have a unison" in {
      // Given
    ...
    "map scales with negative intervals" in {
    ...
    "fail to map a scale with conflicting pitches when the conflicts can't be avoided" in {
    ...
    "map a tetrachord with quarter tones differently based on the shouldMapQuarterTonesLow parameter" in {
    ...
    "map quarter tones by taking quarter-tone tolerance into account" in {
    ...
    "avoid conflicts with quarter-tones when mapping them to opposite direction is possible" in {
    ...
    "fail when conflicts with quarter-tones can't be avoided by mapping them to opposite direction" in {
    ...
    "map a scale with softChromaticGenusMapping set to off" in {
      // Given
    ...
    "map a scale with softChromaticGenusMapping set to strict" in {
      // Given
      val mapperWithLowQuarterTones = AutoTuningMapper(shouldMapQuarterTonesLow = true,
    ...
    "map a scale with softChromaticGenusMapping set to pseudoChromatic" in {
      // Given
      val mapperWithLowQuarterTones = AutoTuningMapper(shouldMapQuarterTonesLow = true,
    ...
    "map a scale on different pitch classes based on basePitchClass parameter" in {
    ...
    "manually map some pitches mentioned in overrideKeyboardMapper" in {
      // Given
    ...
    "avoid conflict with manually mapped pitches" in {
      // Given
  ...
  "mapInterval" should {
    "map an interval to a pitch class with an offset in cents" in {
      val tolerance = 10
  ...
  "keyboardMappingOf" should {
    "return the keyboard mapping automatically found" in {
      // Given
      val major = RatiosScale((1, 1), (9, 8), (5, 4), (4, 3), (3, 2), (5, 3), (15, 8), (2, 1))
      val tuningReference = ConcertPitchTuningReference(32 /: 27, MidiNote(PitchClass.C, 5))
      // When
    ...
    "return the keyboard mapping with some pitches manually mapped" in {
      // Given
      val scale = RatiosScale(1 /: 1, 12 /: 11, 32 /: 27, 4 /: 3, 3 /: 2, 13 /: 8, 16 /: 9)
      val tuningReference = ConcertPitchTuningReference(2 /: 3, MidiNote.D4)
