...
class TuningSeqMappingIntegrationTest extends AnyWordSpec with Matchers {
  ...
  "Tuning sequence mapping" should {
    "successfully create a tuning sequence out of 'minor-major.mtlist' file" in {
    ...
    "successfully create a tuning sequence out of \"La Cornu\" composition  file" in {
    ...
    "successfully create a tuning sequence for a a composition with global settings, baseUrl and a $ref" in {
