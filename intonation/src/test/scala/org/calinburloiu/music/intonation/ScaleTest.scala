...
class ScaleTest extends AnyWordSpec with Matchers {
  ...
  "apply" should {
    "return the interval at the specified index" in {
  ...
  "equals" should {
    "tell if two scales have the same name and the same intervals" in {
  ...
  "size" should {
    "tell how many intervals the scale has" in {
      allScales.foreach(_.size shouldEqual 4)
    }
  }

  "range" should {
    "tell the interval between the first and the last step" in {
  ...
  "direction" should {
    "tell whether the scale is ascending or descending" in {
  ...
  "relativeIntervals" should {
    "return the intervals between all pairs of the adjacent absolute intervals" in {
  ...
  "transpose" should {
    "add an interval to all scale pitches" in {
  ...
  "rename" should {
    "change the name of the scale" in {
  ...
  "intonationStandard" should {
    "return Some IntonationStandard if there is a consistent one" in {
  ...
  "isCentsScale" should {
    "tell if it's a scale that only has cents intervals" in {
  ...
  "isRatiosScale" should {
    "tell if it's a scale that only has ratio intervals" in {
  ...
  "isEdoScale" should {
    "tell if it's a scale that only has EDO intervals" in {
  ...
  "create with no intonationStandard" should {
    "create a scale with the correct type" in {
      Scale.create("mixed",
    ...
    "sort intervals in ascending order" in {
  ...
  "create with intonationStandard" should {
    "create a scale with the correct type" in {
      val edo = EdoIntervalFactory(72)
  ...
  "convertToIntonationStandard" should {
    "convert a scale if necessary to a given intonation standard" in {
      // When
  ...
  "indexOfUnison" should {
    "tell the index where a unison interval is in the scale's intervals, if any" in {
