...
class RealIntervalTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  ...
  "a RealInterval" should {
    "have a the real value greater than 0" in {
  ...
  "realValue" should {
    "return the same decimal value that was passed when the object was created" in {
  ...
  "cents" should {
    "return the value of the interval in cents" in {
  ...
  "normalizing" should {
    "put the interval between unison (inclusive) and octave (exclusive)" in {
      //@formatter:off
  ...
  "+" should {
    "add two intervals musically (on the logarithmic scale)" in {
      // Neutral element
  ...
  "-" should {
    "subtract two intervals musically (on the logarithmic scale)" in {
      // Neutral element
      (RealInterval.Unison - RealInterval.Unison) shouldEqual RealInterval.Unison
  ...
  "*" should {
    "repeatedly add an interval to itself musically (on the logarithmic scale)" in {
      // Neutral element
  ...
  "invert" should {
    "compute the interval inversion" in {
  ...
  "reverse" should {
    "compute the ratio useful for reproducing intervals on vibrating strings" in {
      // Neutral element
  ...
  "isUnison" should {
    "correctly report if the interval is a unison" in {
      RealInterval(1.0).isUnison should be(true)
      RealInterval(1.5).isUnison should be(false)
    }
  }

  "toRealInterval" should {
    "convert the interval to a RealInterval" in {
      val interval = RealInterval(1.5)
      interval.toRealInterval shouldBe theSameInstanceAs(interval)
    }
  }

  "toCentsInterval" should {
    "convert the interval to a CentsInterval" in {
...
class RatioIntervalTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  private val epsilon: Double = 1e-2
  private implicit val doubleEquality: Equality[Double] = TolerantNumerics.tolerantDoubleEquality(epsilon)

  "a RatioInterval" should {
    "have the numerator and denominator greater than 0" in {
  ...
  "realValue" should {
    "return the decimal value of the frequency ratio" in {
  ...
  "harmonicSeriesOf" should {
    "create a series with the harmonics that match for a set of ratio intervals" in {
...
class CentsIntervalTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  private val epsilon: Double = 1e-2
  private implicit val doubleEquality: Equality[Double] = TolerantNumerics.tolerantDoubleEquality(epsilon)
  ...
  "cents" should {
    "return the same cents value that was passed when the object was created" in {
  ...
  "toCentsInterval" should {
    "convert the same CentsInterval instance" in {
...
class EdoIntervalTest extends AnyWordSpec with Matchers with TableDrivenPropertyChecks {
  private val epsilon: Double = 1e-2
  private implicit val doubleEquality: Equality[Double] = TolerantNumerics.tolerantDoubleEquality(epsilon)

  "an EdoInterval" should {
    "have edo value as a natural number" in {
      assertThrows[IllegalArgumentException] {
        EdoInterval(0, 3)
      }
      assertThrows[IllegalArgumentException] {
  ...
  "an EdoInterval relative to standard 12-EDO" should {
    "be created" in {
  ...
  "1200-EDO intervals" should {
    "give cents values equal to their counts" in {
  ...
  "EdoIntervalFactory" should {
    "allow easy creation of intervals with the same EDO value" in {
...
class IntervalTest extends AnyWordSpec with TableDrivenPropertyChecks with Matchers {
  private val epsilon: Double = 1e-2
  private implicit val doubleEquality: Equality[Double] = TolerantNumerics.tolerantDoubleEquality(epsilon)
