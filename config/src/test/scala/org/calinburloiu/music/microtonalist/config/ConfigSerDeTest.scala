...
class ConfigSerDeTest extends AnyWordSpec with Matchers {

  "createHoconValue" should {
    "create HOCON singleton values" in {
    ...
    "create HOCON lists from scala Seq" in {
    ...
    "create HOCON objects from scala Map" in {
      val m1 = Map("a" -> 1, "b" -> 3)
  ...
  "withAnyRefValue" should {
    "not change HOCON object if requesting to replace a value that already exists" in {
    ...
    "change HOCON object with a new value" in {
      val m1 = Map("a" -> 1, "b" -> 3).asJava
      val hocon = ConfigFactory.parseMap(m1)
