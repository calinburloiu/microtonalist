...
* (d) When there are multiple active notes on an output Member Channel, their Channel Pressure Expression Value (that originates from a Polyphonic Key Pressure) is averaged, similar with MPE Input Mode, but dissimilar to it, preserving its latest value on Note Off does have any effect since on the next Note On on that channel, the value must be reset to 0. Remember that it must be reset because it originates from a Polyphonic Key Pressure which always has a 0 value on Note On.
