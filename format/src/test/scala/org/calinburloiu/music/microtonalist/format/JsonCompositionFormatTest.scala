...
class JsonCompositionFormatTest extends AnyWordSpec with Matchers with Inside with BeforeAndAfter with MockFactory {
  ...
  "JsonCompositionFormat" should {
    "take name from tuning spec context when reading an inline scale" in {
    ...
    "successfully read 72-EDO intervals in 72-EDO intonation standard" in {
    ...
    "successfully read an in-context scale with just intonation intervals in 72-EDO intonation standard" in {
      val composition = readCompositionFromResources("format/72-edo.mtlist", compositionRepo)
    ...
    "fail to read an in-context scale with EDO intervals in just intonation standard" in {
    ...
    "fail to read a referenced .jscl scale with EDO intervals in just intonation standard" in {
    ...
    "fail to read a referenced .scl scale with cents intervals in just intonation standard" in {
      assertThrows[IncompatibleIntervalsScaleFormatException] {
        readCompositionFromResources("format/intonation-standard-incompatibility-scl.mtlist", compositionRepo)
      }
    }

    "successfully read a valid composition file" in {
    ...
    "convert intervals/scales according to the intonation standard" in {
    ...
    "read scales with the same URI once" in {
    ...
    "read a default TuningMapper from settings if omitted in TuningSpec" in {
      val composition = readCompositionFromResources("format/default-tuningMapper-from-settings.mtlist",
        compositionRepo)
    ...
    "populate the URL of a composition when reading it from one" in {
      val composition = readCompositionFromResources("format/minor-major.mtlist", compositionRepo)
