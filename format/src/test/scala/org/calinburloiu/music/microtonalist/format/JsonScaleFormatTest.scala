...
class JsonScaleFormatTest extends JsonFormatTestUtils {
  ...
  "pitchIntervalFormatFor" should {
    "either read on interval directly or get it from a scale pitch object" in {
  ...
  "Reading a JSON Scale in cents intonation standard" should {
    "create a CentsScale object when all intervals are in cents" in {
      // When
    ...
    "create a Scale object when some intervals are ratios" in {
      // Given
      val scale = Json.obj(
        "name" -> "mix",
    ...
    "fail if there are intervals written with EDO-style arrays" in {
      // Given
      val scale = Json.obj(
        "name" -> "bad",
        "intonationStandard" -> Json.obj("type" -> "cents"),
    ...
    "fail if some intervals are invalid" in {
  ...
  "Writing a scale with intervals in cents" should {
    "output a JSON Scale with cents intonation standard" in {
      // When
  ...
  "Reading a JSON Scale in just intonation standard" should {
    "create a RatiosScale object when all intervals are ratios" in {
      // When
    ...
    "fail if there are intervals that are not expressed as ratios" in {
  ...
  "Writing a scale with intervals as ratios" should {
    "output a JSON Scale with just intonation standard" in {
      // When
  ...
  "Reading a JSON Scale in EDO intonation standard" should {
    "create an EdoScale object when all intervals are expressed as absolute values in divisions" in {
      // When
    ...
    "create an EdoScale object when some intervals are expressed relative to 12-EDO" in {
      // When
    ...
    "create a EdoScale object when some intervals are given as ratios" in {
      // Given
      val json = Json.obj(
        "name" -> "hicaz-4",
        "intonationStandard" -> Json.obj("type" -> "edo", "countPerOctave" -> 72),
      ...
      // Then
      result shouldBe a[EdoScale]
      result shouldEqual Scale("hicaz-4",
        edo72(-2, 0), edo72(0, 0), edo72(1, +1), edo72(4, -1), edo72(5, 0), edo72(6, 1))
    }
  ...
  "Writing scales with mixed interval types" should {
    "fail if no context is provided" in {
  ...
  "Reading an in-context JSON Scale without a name" should {
    "fail if the context does not provide one" in {
      assertThrows[MissingContextScaleFormatException] {
        scaleFormat.read(scaleJsonWithoutName, None)
      }
    }

    "take the name from the context" in {
      // Given
  ...
  "Reading an in-context JSON Scale without intonation standard" should {
    "fail if the context does not provide one" in {
      assertThrows[MissingContextScaleFormatException] {
    ...
    "take the intonation standard from the context" in {
      // Given
      val context = Some(ScaleFormatContext(intonationStandard = Some(JustIntonationStandard)))
      // When
  ...
  "Reading a concise JSON Scale" should {
    "fail if the context does not provide a name and the intonation standard" in {
      assertThrows[MissingContextScaleFormatException] {
        scaleFormat.read(min4JsonIntervals, None)
      }
    }

    "take the name and the intonation standard from the context" in {
      // Given
  ...
  "Reading a JSON Scale with extra metadata" should {
    "successfully create a Scale instance" in {
      // Given
      val json = Json.obj(
        "name" -> "hicaz-4",
        "authorName" -> "John Doe",
